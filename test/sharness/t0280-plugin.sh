...
test_expect_success "noplugin flag works" '
  test_must_fail go run -tags=noplugin github.com/ipfs/go-ipfs/cmd/luanet id > output 2>&1
  test_should_contain "not built with plugin support" output
'

test_expect_success "noplugin flag works" '
  CGO_ENABLED=0 test_must_fail go run github.com/ipfs/go-ipfs/cmd/luanet id > output 2>&1
