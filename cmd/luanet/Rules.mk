...
$(d)-try-build $(IPFS_BIN_$(d)): GOFLAGS += $(cmd/luanet_flags)
...
$(d)-install: GOFLAGS += $(cmd/luanet_flags)
$(d)-install: $(d) $$(DEPS_GO) ALWAYS
	$(GOCC) install $(go-flags-with-tags) ./cmd/luanet
.PHONY: $(d)-install
...
$(COVER_BIN_$(d)): $(d) $$(DEPS_GO) ALWAYS
	$(eval TMP_PKGS := $(shell $(GOCC) list -f '{{range .Deps}}{{.}} {{end}}' $(go-flags-with-tags) ./cmd/luanet | sed 's/ /\n/g' | grep ipfs/kubo) $(call go-pkg-name,$<))
